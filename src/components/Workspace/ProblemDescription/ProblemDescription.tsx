...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({
  ...
  const returenUserDataAndProblemData = async (transaction: any) => {
    const userRef = doc(firestore, "users", user!.uid);
    const problemRef = doc(firestore, "problems", problem.id);
    const userDoc = await transaction.get(userRef);
    const problemDoc = await transaction.get(problemRef);

    return { userRef, problemDoc, userDoc, problemRef };
  };
  const handleLike = async () => {
    if (!user) {
      toast.error("You must be logged in like a problem", {
        position: "top-left",
        theme: "dark",
      });
      return;
    ...
    await runTransaction(firestore, async (transaction) => {
      const { problemDoc, userDoc, userRef, problemRef } =
        await returenUserDataAndProblemData(transaction);
  ...
  return (
    <Container>
      <div className="">
        {/* TAB */}
        <div className="description_block flex h-11 w-full items-center pt-2  overflow-x-hidden">
          <div
            className={
              "description_div rounded-t-[5px] px-5 py-[10px] text-xs cursor-pointer"
            }
          >
            Description
          </div>
        ...
        <div className="flex px-0 py-4 h-[calc(100vh-104px)] overflow-y-auto">
          <div className="px-5">
            {/* Problem heading */}
